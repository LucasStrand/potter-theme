"use client";
// Standalone multi-theme wallpaper studio (/tools/wallpaper-converter).
// Re-inks any image onto a chosen theme's palette (Potter, Catppuccin, Gruvbox,
// Dracula, Nord, Tokyo Night, Rosé Pine) and dresses the whole page in the
// selected flavor. Shares the recolor core + worker with the homepage tool.
...
export function WallpaperStudio() {
  const [flavorId, setFlavorId] = useState("potter-quill");
  const flavor = useMemo(() => findFlavor(flavorId), [flavorId]);
  const theme = useMemo(() => themeOf(flavorId), [flavorId]);
  const targets = useMemo(() => flavorTargets(flavor), [flavor]);
  const ui = flavor.ui;
  const onAccent = readableOn(ui.accent);
  ...
  // Paint the document background to match so overscroll/edges stay on-theme.
  useEffect(() => {
    const el = document.documentElement;
    const prev = el.style.background;
    el.style.background = ui.bg;
    return () => {
  ...
  const opts = useCallback(
    (): ConvertOptions => ({ mode, strength, softness, preserveLuminance, targets }),
    [mode, strength, softness, preserveLuminance, targets],
  );
  ...
  // colorful sample so the tool is alive on first paint
  useEffect(() => {
  ...
  // recompute preview when source / options / flavor change (debounced)
  ...
  const download = useCallback(async () => {
    ...
    try {
      ...
      tmp.getContext("2d")!.putImageData(new ImageData(new Uint8ClampedArray(out), full.width, full.height), 0, 0);
      const blob = await new Promise<Blob | null>((res) => tmp.toBlob(res, "image/png"));
      if (blob) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `${fileName}-${flavor.id}.png`;
        a.click();
  ...
  }, [convert, opts, fileName, flavor.id]);
    ...
    >
      <div className="mx-auto w-full max-w-6xl px-6 py-8 sm:py-12">
        ...
        <header className="mt-10 sm:mt-14">
          <p className="font-mono text-[11px] uppercase tracking-[0.28em]" style={{ color: "var(--faint)" }}>
            wallpaper studio
          </p>
          <h1 className="font-display mt-3 text-3xl font-semibold sm:text-5xl" style={{ color: "var(--text)" }}>
            Re-ink any wallpaper
          </h1>
          <p className="mt-3 max-w-2xl text-base sm:text-lg" style={{ color: "var(--subtext)" }}>
            Drop in any image and recolor it onto your favorite palette. Pick a theme below — the whole page
            wears it while you work.
          </p>
        </header>

        {/* theme + flavor pickers */}
          ...
          <div className="flex flex-wrap gap-1.5">
            {THEMES.map((t) => {
              ...
              return (
                <button
                  key={t.id}
                  onClick={() => setFlavorId(t.flavors[Math.min(1, t.flavors.length - 1)].id)}
            ...
            {theme.flavors.map((f) => {
              ...
              return (
                <button
                  ...
                  className="flex cursor-pointer items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium transition-colors"
                  style={{
                    background: active ? "var(--accent)" : "var(--panel)",
                    color: active ? "var(--on-accent)" : "var(--subtext)",
                    border: "1px solid var(--panel-border)",
                ...
                >
                  <span
                    ...
                    style={{ background: f.ui.accent, outline: "1px solid rgba(127,127,127,.35)" }}
                  />
                  {f.label}
                </button>
              );
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_300px]">
          ...
          >
            ...
            <span
              ...
              style={{ background: "rgba(0,0,0,0.45)", color: "#f4f1ea" }}
            >
              original
            </span>
            <span
            ...
            <div
              ...
              style={{ left: `${divider}%` }}
            >
              <span className="h-full w-px" style={{ background: "rgba(255,255,255,0.6)" }} />
              <span
            ...
            {busy && (
              <span
                ...
                style={{ background: "rgba(0,0,0,0.5)", color: "#f4f1ea" }}
              >
                inking…
              </span>
            )}
          ...
          <aside className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="flex gap-1.5">
                <Pill active={mode === "smooth"} onClick={() => setMode("smooth")}>Smooth</Pill>
                <Pill active={mode === "nearest"} onClick={() => setMode("nearest")}>Nearest</Pill>
              </div>
              <p className="text-[11px]" style={{ color: "var(--faint)" }}>
                {mode === "smooth"
                  ? "Blends toward the palette — keeps gradients and detail."
                  : "Snaps every pixel to the closest color — bold and flat."}
              </p>
            </div>
            ...
            {mode === "smooth" && (
              <>
                ...
                <label className="flex cursor-pointer items-center gap-2 text-sm" style={{ color: "var(--subtext)" }}>
                  <input
                    type="checkbox"
            ...
            <div className="space-y-2 pt-2">
              ...
              <p className="text-center text-[11px]" style={{ color: "var(--faint)" }}>
  ...
  function Label({ children }: { children: React.ReactNode }) {
    return (
      <p className="font-mono text-[11px] uppercase tracking-[0.18em]" style={{ color: "var(--faint)" }}>
        {children}
      </p>
    );
  }
    ...
    return (
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <Label>{label}</Label>
          <span className="font-mono text-[11px]" style={{ color: "var(--subtext)" }}>
            {format(value)}
          </span>
        </div>
        <input
